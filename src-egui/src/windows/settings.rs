...
pub fn show(
    ...
    egui::CentralPanel::default().show(ctx, |ui| {
        ...
        // ── Tab bar ──────────────────────────────────────────────────────────
        ui.horizontal(|ui| {
            for (i, label) in ["Dashboard", "Panels", "Alerts", "Appearance"]
                .iter()
                .enumerate()
            {
...
fn draw_alerts(ui: &mut egui::Ui, draft: &mut settings::Settings) {
    ...
    ui.add_space(4.0);
    ui.checkbox(
        &mut draft.notify_on_crit,
        "Notify when critical threshold is exceeded",
    );
    ui.add_space(8.0);
    ...
    for &(key, label) in &[
        ...
        egui::Grid::new(format!("t_{key}"))
            ...
            .show(ui, |ui| {
                ...
                ui.add_enabled(
                    warn_on,
                    egui::DragValue::new(&mut warn_val)
                        .range(0..=255)
                        .suffix(unit),
                );
                ui.end_row();
                ...
                ui.add_enabled(
                    crit_on,
                    egui::DragValue::new(&mut crit_val)
                        .range(0..=255)
                        .suffix(unit),
                );
                ui.end_row();
