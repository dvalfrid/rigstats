use egui::{Align2, Rect, RichText, Sense, Ui, Vec2, pos2};
...
fn paint_row(
  ...
  ui.painter().with_clip_rect(name_clip).text(
    pos2(x0, cy), Align2::LEFT_CENTER, name, font_id.clone(), colors[0],
  );

  // CPU — right-aligned inside its quarter, with inner padding
  let cpu_right = x0 + name_w + cpu_w - CELL_PAD;
  ui.painter().text(
    pos2(cpu_right, cy), Align2::RIGHT_CENTER, cpu, font_id.clone(), colors[1],
  );

  // RAM — right-aligned flush to row right edge (with padding)
  ui.painter().text(
    pos2(rect.max.x - CELL_PAD, cy), Align2::RIGHT_CENTER, ram, font_id, colors[2],
  );
}
...
pub fn draw(ui: &mut Ui, stats: &PollStats, opacity: f32) {
  ...
  theme::panel_frame(ui, theme::C_PROC, opacity, |ui| {
    ...
    ui.label(RichText::new("PROCESSES").strong().color(theme::C_PANEL_TITLE).size(theme::FONT_PANEL_TITLE));
    ...
    ui.add_space(2.0);
    paint_row(ui, inner_w, "NAME", "CPU", "RAM",
      [theme::C_STAT_LABEL; 3]);
    ui.add_space(2.0);
