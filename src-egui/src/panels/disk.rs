...
pub fn draw(ui: &mut Ui, stats: &PollStats) {
  theme::panel_frame(ui, theme::C_PUR, |ui| {
    ...
    ui.label(
      RichText::new("DISK").strong().color(theme::C_PANEL_TITLE).size(theme::FONT_PANEL_TITLE),
    );
    ui.add_space(4.0);
    ...
    let scroll_h =
      n_visible as f32 * ROW_H + (n_visible.saturating_sub(1)) as f32 * ROW_GAP;
        ...
        for drive in drives.iter() {
          ...
          ui.horizontal(|ui| {
            ui.allocate_ui_with_layout(
              ...
              |ui| { ui.label(RichText::new(&drive.fs).small().color(theme::C_TEXT)); },
            );
            ...
            ui.allocate_ui_with_layout(
              ...
              |ui| {
                ui.label(
                  RichText::new(format!("{}%", drive.pct)).small().color(theme::C_TEXT),
                );
              },
            );
            ui.allocate_ui_with_layout(
