...
pub fn draw(ui: &mut Ui, stats: &PollStats, opacity: f32) {
  theme::panel_frame(ui, theme::C_MB, opacity, |ui| {
    ...
    ui.horizontal(|ui| {
      ui.label(RichText::new("MOTHERBOARD").strong().color(theme::C_PANEL_TITLE).size(theme::FONT_PANEL_TITLE));
    });
    ...
    if !stats.mb_fans.is_empty() || !stats.mb_temps.is_empty() || !stats.mb_voltages.is_empty() {
      ...
      egui::ScrollArea::vertical()
        .id_salt("mb_data")
        .max_height(data_h)
        .show(ui, |ui| { ui.columns(3, |cols| {
        // Col 0: FAN
        if !stats.mb_fans.is_empty() {
          egui::Grid::new("mb_fans").num_columns(2).min_col_width(24.0).show(&mut cols[0], |ui| {
        ...
        if !stats.mb_temps.is_empty() {
          egui::Grid::new("mb_temps").num_columns(2).min_col_width(24.0).show(&mut cols[1], |ui| {
            ui.label(RichText::new("SENSOR").small().color(theme::C_STAT_LABEL));
            ui.label(RichText::new("°C").small().color(theme::C_STAT_LABEL));
            ui.end_row();
            for (label, t) in &stats.mb_temps {
              ui.label(RichText::new(short_label(label)).small().color(theme::C_TEXT_MUTED));
              ui.label(RichText::new(format!("{t:.0}")).small().color(temp_color(Some(*t), 70, 90)));
              ui.end_row();
            }
          });
        }
        ...
        if !stats.mb_voltages.is_empty() {
          egui::Grid::new("mb_volts").num_columns(2).min_col_width(24.0).show(&mut cols[2], |ui| {
