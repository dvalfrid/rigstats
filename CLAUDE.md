...
- **`renderer/panels/`** — One file per panel: `cpu.js`, `gpu.js`, `ram.js`, `network.js`, `disk.js`. Each exports an `update*Panel(stats, history, pushHistory, thresholds)` function. `thresholds` carries `{ warn, crit }` for temperature colour mapping; defaults apply in browser/simulator mode. `disk.js` cycles through pages of three drives every 5 ticks when more than three drives are present; the page resets automatically when the drive count changes.
