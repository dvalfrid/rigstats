...
- **`renderer/panels/`** — One file per panel: `cpu.js`, `gpu.js`, `ram.js`, `network.js`, `disk.js`, `motherboard.js`. Each exports an `update*Panel(stats, ...)` function. `thresholds` carries `{ warn, crit }` for temperature colour mapping; defaults apply in browser/simulator mode. `disk.js` cycles through pages of three drives every 5 ticks when more than three drives are present; the page resets automatically when the drive count changes. `motherboard.js` renders fans/temps/voltages in a three-column layout; `shortLabel()` maps `"Temperature #N"` → `"TN"` and truncates other labels to fit the `8ch` CSS grid column.
