...
- **`windows/`** — one file per secondary window (`settings.rs`, `about.rs`, `status.rs`, `updater.rs`). All use `egui::show_viewport_deferred` and are centered via `dialog_center()` in `main.rs`. All four viewports receive the tray icon via `load_app_icon()` (loads `assets/tray.png` as `egui::IconData`). `settings.rs` holds a five-tab layout (Display, Panels, Alerts, Appearance, General) with live preview: changes are pushed to `current_settings` every frame when `draft != last_preview`; Cancel restores the `original` snapshot captured at open; Save also persists to disk. **Tab contents:** *Display* = display profile, Window (window layer + opacity), Layout (floating mode + panel scale, fill screen + alignment); *Panels* = panel visibility + System Power (PSU capacity); *Alerts* = notifications + temperature/battery thresholds; *Appearance* = override model name + theme preset; *General* = launch at startup + stats logging. All window-placement controls live on the **Display** tab so the wallpaper-driven gray-out is visible right below the layer that causes it. **Live preview applies to:** opacity, theme, rig name, visible panels, floating mode, display profile, all threshold values. **Save-only (not previewed live):** `window_layer`. When the **applied** (currently running) `window_layer` is `"wallpaper"`, Settings disables the Display-tab controls that have no effect (or shouldn't change live) in wallpaper mode — **Display Profile** (shown as a static label), opacity (Window card), Floating Mode and Fill Screen + Alignment (Layout card) — via `ui.add_enabled_ui(!is_wallpaper, …)` / a disabled label. The gate uses the *applied* layer (`original.window_layer`, passed into `draw_display`) rather than the draft, because `window_layer` is Save-only: the app stays in wallpaper mode until Save even after the draft picks Normal, so gating on the draft would falsely re-enable controls before the switch took effect. A `wallpaper_save_banner` (amber ⚠) is shown across **all** tabs while applied == wallpaper, since the host renders from disk so nothing previews live — everything applies on Save. The Display Profile is locked because it can't preview live there; change it in a non-wallpaper layer, then switch to wallpaper. When the draft layer differs from the applied layer a separate "⚠ Window Layer changes take effect when you click Save." note (yellow icon) is shown. `status.rs` shows a two-column Components row (`render_components`): a Dependencies card (PawnIO, sidecar service, WMI) on the left and a GPU Drivers card on the right listing each adapter's installed driver version + date, an age-based "stale driver" warning (`DRIVER_STALE_DAYS = 270`), and a per-adapter right-aligned "↗ Latest driver" link to the vendor support page (AMD/NVIDIA/Intel). `render_components` measures the Dependencies card height (`min_rect` delta) and forces the Drivers card to the same height; the driver list sits inside an `egui::ScrollArea` (unique `id_salt("gpu_drivers_scroll")` to avoid an ID clash with the debug-log scroll area) so extra GPUs scroll while the card height stays fixed. Driver data comes from `hardware::detect_gpu_drivers()`. Outdated GPU drivers are the known cause of missing ADL GPU sensors (LHM #736) — see `docs/troubleshooting.md`.
