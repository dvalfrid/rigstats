...
- **`commands.rs`** — Thin `#[tauri::command]` handlers only. Each handler delegates to a domain module; no business logic lives here. Floating mode transitions are serialized with a mutex in `toggle_floating_mode` to prevent overlapping enable/disable races. `notify_app_ready` triggers `prewarm_panel_windows` (when not already in floating mode) so WebView2 windows are ready before the user first enables floating mode. Includes `set_gpu_preference` command (accepts both `gpu_name` and `gpuName`) used by the GPU selector dots in fixed and floating mode.
