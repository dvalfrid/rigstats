...
$features = @(
  ...
  @{ id="session-history"; kind="done"; label="enhancement"; title="Session history: record, browse, and visualize past sessions";
     summary="Turned stats logging into named, browsable recording sessions: explicit start/stop via the existing tray Start/Stop Recording toggle (now with a blinking tray icon and procedurally-drawn menu icons), a per-session CSV file instead of the daily-rolling log, and a History window (egui_plot charts of CPU/GPU/RAM/disk/net/ping, synced hover) to pick a past session and see what happened. Sessions can be pinned to survive retention pruning, renamed, revealed, and deleted. The session index is hardened against corruption (backup + recovery) and cross-process races (main app vs. rigstats-wallpaper) with a file lock.";
     status="Shipped on main - 429f1b8d6, releasing in v1.38.0 (not yet released)" }
