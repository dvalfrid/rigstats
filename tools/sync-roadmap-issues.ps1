...
$features = @(
  ...
  @{ id="background-transparency"; kind="planned"; label="enhancement"; title="Background-only transparency (per-pixel alpha)"; milestone="v3.0";
     summary="Goal: transparent panel backgrounds with opaque text/graphs/bars/images, in the Normal/Always-on-Top/Always-Behind window layers (Settings/About/Updater dialogs stay fully opaque). Originally dropped as blocked - four approaches failed (wgpu/glow transparent, two LWA_COLORKEY variants). Unblocked by #131/#168 and implemented: main.rs now forces the DX12 backend with wgpu::Dx12SwapchainKind::DxgiFromVisual, applies WS_EX_NOREDIRECTIONBITMAP post-window-creation (win_opacity::set_no_redirection_bitmap), and premultiplies clear_color/panel fills by opacity instead of the old WS_EX_LAYERED whole-window dim. Verified visually in Normal/Always-on-Top/Always-Behind: background fades, text/gauges/bars stay crisp, no regressions. Dialog viewports (Settings/About/Status/Updater) never call with_transparent so they should stay opaque by construction - pending a live click-through confirmation. Floating mode is a separate follow-up: see background-transparency-floating.";
     status="Implemented for Normal/Always-on-Top/Always-Behind; pending live dialog-opacity confirmation before closing" }
  @{ id="background-transparency-floating"; kind="planned"; label="enhancement"; title="Extend selective per-pixel transparency (DComp) to floating mode"; milestone="v3.0";
     summary="Floating mode renders one separate OS viewport per visible panel (show_viewport_immediate), so the DComp technique from background-transparency (main window only) needs repeating per panel viewport rather than once - HWND-caching, win_opacity::set_no_redirection_bitmap, premultiplied clear_color per viewport, verified safe under panels being created/destroyed as visibility toggles. Floating panels currently keep the old WS_EX_LAYERED uniform-dim opacity, unaffected either way.";
     status="Planned (3.0)" }
