...
fn parse_sessions_index(raw: &str) -> Option<Vec<SessionMeta>> {
    serde_json::from_str::<SessionsIndex>(raw)
        .ok()
        .map(|i| i.sessions)
}
        ...
        Ok(raw) => match parse_sessions_index(&raw) {
            ...
            None => {
                crate::debug::log_error(
                    dir,
                    "logging: sessions index is corrupt — recovering from backup",
                );
...
fn log_persist_err(dir: &Path, e: &str) {
    crate::debug::log_error(
        dir,
        &format!("logging: failed to persist session index — {e}"),
    );
}
        ...
        if let Err(e) = fs::remove_file(&path) {
            if e.kind() != std::io::ErrorKind::NotFound {
                crate::debug::log_warn(
                    dir,
                    &format!(
                        "logging: failed to delete session file {} — {e}",
                        path.display()
                    ),
                );
            }
...
pub fn prune_old_sessions(dir: &Path, days: u32) {
    ...
    for s in sessions.drain(..) {
        let expired = s
            .end_unix
            .map(|end| now.saturating_sub(end) > cutoff)
            .unwrap_or(false);
        if expired && !s.pinned {
            ...
            if let Err(e) = fs::remove_file(&path) {
                if e.kind() != std::io::ErrorKind::NotFound {
                    crate::debug::log_warn(
                        dir,
                        &format!("logging: failed to prune session {} — {e}", s.id),
                    );
                }
            }
        } else {
    ...
    let file = match fs::File::open(path) {
        ...
        Err(e) => {
            ...
            crate::debug::log_warn(
                dir,
                &format!(
                    "logging: failed to open session CSV {} — {e}",
                    path.display()
                ),
            );
            return Vec::new();
...
pub fn summarize_rows(rows: &[SessionRow]) -> SessionSummary {
    ...
    let avg_gpu_load =
        (!gpu_vals.is_empty()).then(|| gpu_vals.iter().sum::<f64>() / gpu_vals.len() as f64);
    let peak_gpu_load = gpu_vals.iter().copied().fold(None, |acc: Option<f64>, v| {
...
pub fn reconcile_sessions_on_startup(dir: &Path) {
    ...
    for s in sessions.iter_mut() {
        if s.end_unix.is_none() {
            ...
            s.end_unix = Some(
                rows.last()
                    .map(|r| r.timestamp_unix)
                    .unwrap_or(s.start_unix),
            );
    ...
    let known_legacy: HashSet<String> = sessions
        .iter()
        .filter(|s| s.legacy)
        .map(|s| s.id.clone())
        .collect();
    if let Ok(entries) = fs::read_dir(dir) {
        for entry in entries.flatten() {
            ...
            let date = name
                .trim_start_matches("rigstats-log-")
...
mod tests {
    ...
    fn fmt_opt_rounds_to_precision() {
        assert_eq!(fmt_opt(Some(9.876), 1), "9.9");
    ...
    fn append_and_end_session_computes_summary() {
        ...
        append_stats_row(
            &sample_payload(40, Some(60.0), 1_073_741_824),
            dir.path(),
            &meta,
        )
        .unwrap();
        append_stats_row(
            &sample_payload(60, Some(80.0), 2_147_483_648),
            dir.path(),
            &meta,
        )
        .unwrap();
    ...
    fn missing_session_csv_is_logged_not_silent() {
        ...
        let log = fs::read_to_string(crate::debug::debug_log_path(dir.path()))
            .expect("debug log must exist");
        assert!(
    ...
    fn prune_keeps_pinned_and_active_but_removes_expired() {
        ...
        let remaining: Vec<String> = load_sessions(dir.path())
            .into_iter()
            .map(|s| s.id)
            .collect();
        assert!(
            !remaining.contains(&expired.id),
            "expired unpinned session must be pruned"
        );
        assert!(
            remaining.contains(&pinned.id),
            "pinned session must survive pruning"
        );
        assert!(
            remaining.contains(&active.id),
    ...
    fn load_sessions_recovers_from_backup_when_index_is_corrupt() {
        ...
        set_session_pinned(dir.path(), &meta.id, true);
        assert!(
            backup_index_path(dir.path()).exists(),
            "backup must exist after a save"
        );
        ...
        assert_eq!(
            recovered.len(),
            1,
            "must recover the session from the backup"
        );
    ...
    fn lock_waits_for_release_then_succeeds() {
        ...
        let handle = std::thread::spawn(move || {
            ...
            assert!(
                second.held,
                "must wait for the first lock to be released, not give up"
            );
        });
    ...
    fn concurrent_pin_and_rename_do_not_lose_either_update() {
        ...
        assert_eq!(
            sessions.len(),
            1,
            "the session must not be duplicated or dropped"
        );
        assert!(
            sessions[0].pinned,
            "the pin from the other thread must not be lost"
        );
    }
}
